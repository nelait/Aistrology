...
import { computeFriendship, FriendshipResult, FriendshipFactor } from "../astro/friendship";
import { friendshipGrounding, factorGrounding } from "../astro/friendshipReferences";
import { Chart } from "../astro/types";
import { RASHIS, NAKSHATRAS } from "../astro/constants";
import Justify from "./Justify";
import FriendshipExportReport from "./FriendshipExportReport";
...
const VERDICT_COLORS: Record<string, string> = {
  "Soul Mates": "#a855f7",
  "Great Friends": "#22c55e",
  "Good Friends": "#3b82f6",
  Acquaintances: "#eab308",
};
...
function FactorBar({ factor }: { factor: FriendshipFactor }) {
...
export default function FriendshipView({ currentChart, chartOptions, style }: Props) {
  const { user } = useAuth();
  ...
  const result = useMemo<FriendshipResult | null>(() => {
    if (!chart1 || !chart2) return null;
    return computeFriendship(chart1, chart2);
  }, [chart1, chart2]);

  const overallGrounding = useMemo(() => {
    if (!result || !chart1 || !chart2) return null;
    return friendshipGrounding(result, chart1, chart2);
  ...
  if (!user) {
    return (
      <div className="match-empty">
        ...
        <h2>Friendship Match</h2>
        <p>
          Sign in and save at least two birth charts to compute friendship
          compatibility.
        </p>
    ...
    <div className="match-view">
      ...
      <div className="match-selector">
        <div className="match-selector-col">
          <label className="match-role">
            <span className="match-role-icon">👤</span> Person 1
          </label>
        ...
        <div className="match-vs" style={{ color: "#a855f7" }}>⚡</div>

        <div className="match-selector-col">
          <label className="match-role">
            <span className="match-role-icon">👤</span> Person 2
          </label>
      ...
      {result && chart1 && chart2 && (
        <>
          ...
          {/* Factor Grid */}
          <h3 className="match-section-title">Friendship Factors — 6-dimension breakdown</h3>
          <div className="kuta-grid">
            {result.factors.map((f) => {
              const isExpanded = expandedFactor === f.name;
              const grounding = factorGrounding(f);
              return (
                <div className={`kuta-card${isExpanded ? " expanded" : ""}`} key={f.name}>
                  ...
                  {isExpanded && (
                    <div className="kuta-card-body">
                      <p>{f.detail}</p>
                      <Justify
                        subject={`${f.name} — Friendship compatibility`}
          ...
          {overallGrounding && (
            <div className="match-overall-justify">
              <Justify
                subject={`Friendship compatibility between ${saved1?.label ?? "Person 1"} and ${saved2?.label ?? "Person 2"}`}
                basePrediction={result.verdictDescription}
                facts={overallGrounding.facts}
                references={overallGrounding.references}
                guidelines={[
                  "Examine all 6 friendship factors systematically.",
                  "Highlight which areas create natural synergy and which need conscious effort.",
                  "Suggest how the friends can leverage their strengths and navigate differences.",
                ]}
              />
            </div>
          )}
          ...
          <button className="match-export-btn" onClick={() => setShowExport(true)}>
            ⬇ Export Friendship Report
          </button>

          <FriendshipExportReport
            chart1={chart1}
            chart2={chart2}
            result={result}
            label1={saved1?.label ?? "Person 1"}
            label2={saved2?.label ?? "Person 2"}
            open={showExport}
